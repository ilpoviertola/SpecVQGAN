        ...
        if config_cli.data is None:
            if cfg.data.params[a] is not None:
                if "vggsound.VGGSound" in cfg.data.params.train.target:
                    cfg.data.params[a] = os.path.join(
                        "./data/vggsound/", Path(cfg.data.params[a]).name
                    )
                elif "vas.VAS" in cfg.data.params.train.target:
                    cfg.data.params[a] = os.path.join(
                        "./data/vas/features/*", Path(cfg.data.params[a]).name
                    )
...
def make_folder_for_samples(cfg):
    ...
    for split in cfg.sampler.splits:
        samples_split_dirs[split] = (
            Path(cfg.sampler.model_logdir)
            / f"samples_{cfg.sampler.now}"
...
def load_model_and_dataloaders(cfg, device, is_ddp=False):
    ...
    if (Path(cfg.sampler.model_logdir) / "checkpoints/last.ckpt").exists():
        ...
        ckpt_model = sorted(Path(cfg.sampler.model_logdir).glob("checkpoints/*.ckpt"))[
            -1
        ]
    ...
    dsets.datasets = {
        ...
        if split in cfg.sampler.splits
    }

    # loading the vocoder
    ckpt_vocoder = (
        cfg.lightning.callbacks.image_logger.params.vocoder_cfg.params.ckpt_vocoder
    )
    if ckpt_vocoder:
    ...
    for split, dset in dsets.datasets.items():
        if is_ddp:
            sampler = DistributedSampler(
                dset, dist.get_world_size(), dist.get_rank(), shuffle=False
            )
            num_workers = 0
        else:
            sampler = None
...
def sample_spectrogram(cfg, model, batch):
    ...
    with torch.no_grad():
        ...
        for step in range(start_step, hr_w * hr_h):
            ...
            if model.cond_stage_model.__class__.__name__ in [
                "RawFeatsStage",
                "ClassOnlyStage",
                "FeatsClassStage",
            ]:
...
def save_specs(cfg, specs, samples_split_dirs, model, batch, split, sample_id, vocoder):
    ...
    for i, spec in enumerate(specs):
        ...
        if vocoder is not None:
            wave_from_vocoder = (
                vocoder(torch.from_numpy(spec).unsqueeze(0).to("cuda"))
                .cpu()
                .squeeze()
                .detach()
                .numpy()
            ...
            soundfile.write(
                save_path / f"{vidname}_sample_{sample_id}.wav",
                wave_from_vocoder,
                22050,
                "PCM_24",
            )
...
def sample(gpu_id, cfg, samples_split_dirs, is_ddp):
    ...
    if (
        isinstance(cfg.sampler.samples_per_video, int)
        and cfg.sampler.samples_per_video > 0
    ):
...
def main():
    ...
    if local_rank is not None:
        ...
        dist.init_process_group(
            cfg.sampler.get("dist_backend", "nccl"),
            "env://",
