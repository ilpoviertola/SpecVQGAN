...
class ConditionedSpectrogramDataModuleFromConfig(DataModuleFromConfig):
    ...
    def __init__(
        ...
        num_workers,
        spec_dir_path=None,
        rgb_feats_dir_path=None,
        flow_feats_dir_path=None,
        sample_rate=None,
        mel_num=None,
        spec_len=None,
        spec_crop_len=None,
        random_crop=None,
        replace_feats_with_random=None,
        feat_depth=None,
        feat_len=None,
        feat_crop_len=None,
        crop_coord=None,
        ...
        for name, split in {
            ...
            if split is not None:
                ...
                if (
                    split.target.split(".")[-1].startswith("VGGSoundSpecsCondOnFeats")
                    or split.target.split(".")[-1].startswith("VASSpecsCondOnFeats")
                    or split.target.split(".")[-1].startswith("GHSpecsCondOnFeats")
                ):
...
class VocoderGriffinLim(object):
    ...
    def timeout_handler(signum, frame):
        raise TimeoutError(
            "Bad spec: took too much time to reconstruct the sound from spectrogram"
        )
...
class ImageLogger(Callback):
    ...
    def _visualize_attention(self, attention, scale_by_prior=True):
        if scale_by_prior:
            ...
            attention_uniform_prior = 1 / torch.arange(1, T + 1).view(1, T, 1).repeat(
                B, 1, T
            )
            attention_uniform_prior = (
                attention_uniform_prior.tril().view(B, 1, T, T).to(attention.device)
            )
        ...
        for i, spec in enumerate(specs):
            ...
            if pl_module is not None:
                pl_module.logger.experiment.add_audio(
                    f"{tag}_{i}", wave, pl_module.global_step, self.sample_rate
                )
            # in case we would like to save it on disk
            if save_rec_path is not None:
                try:
                    librosa.output.write_wav(
                        save_rec_path, wave.squeeze(0).numpy(), self.sample_rate
                    )
    ...
    def _testtube(self, pl_module, images, batch, batch_idx, split):
        ...
        for k in images:
            tag = f"{split}/{k}"
            if cond_stage_model in ["ClassOnlyStage", "FeatsClassStage"] and k in [
                "conditioning",
                "conditioning_rec",
            ]:
                ...
                if cond_stage_model == "FeatsClassStage":
                    grid = torchvision.utils.make_grid(
                        images[k]["feature"].unsqueeze(1).permute(0, 1, 3, 2),
                        nrow=1,
                        normalize=True,
                    )
                else:
                ...
                grid = torchvision.utils.make_grid(
                    self._visualize_attention(images[k]), nrow=H, normalize=True
                )
            elif cond_stage_model in [
                "RawFeatsStage",
                "VQModel1d",
                "FeatClusterStage",
            ] and k in ["conditioning", "conditioning_rec"]:
                grid = torchvision.utils.make_grid(
                    images[k].unsqueeze(1).permute(0, 1, 3, 2), nrow=1, normalize=True
                )
            else:
                if self.for_specs:
                    # flipping values along frequency dim, otherwise mels are upside-down (1, F, T)
                    grid = torchvision.utils.make_grid(
                        images[k].flip(dims=(2,)), nrow=1
                    )
                    ...
                    if k not in [
                        ...
                        self._log_rec_audio(
                            images[k], tag, pl_module.global_step, pl_module=pl_module
                        )
                else:
        ...
        for k in images:
            ...
            if self.for_specs:
                ...
                if k not in [
                    ...
                    self._log_rec_audio(
                        images[k], tag, pl_module.global_step, save_rec_path=path
                    )
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        if (
            ...
            and pl_module.first_stage_key != "feature"
        ):
            ...
            for k in images:
                if isinstance(images[k], dict):
                    ...
                    if isinstance(images[k]["feature"], torch.Tensor):
                        ...
                        if self.clamp:
                            images[k]["feature"] = torch.clamp(
                                images[k]["feature"], -1.0, 1.0
                            )
                else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if "Spectrogram" in config.data.target:
            ...
            default_callbacks_cfg["image_logger"]["params"][
                "spec_dir_name"
            ] = spec_dir_name
            default_callbacks_cfg["image_logger"]["params"][
                "sample_rate"
